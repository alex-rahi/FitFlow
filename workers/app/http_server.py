"""HTTP API for synchronous YOLO moderation (local dev + demo pipeline)."""
...
logger = logging.getLogger("gymtok-worker-http")

app = FastAPI(title="GymTok YOLO Worker", version="1.0.0")
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
    allow_methods=["*"],
...
async def analyze_local_path(storage_path: str):
    """Analyze a file already on the shared uploads volume."""
